...
inds = c(2,:)>-71.99 & c(2,:)<-50.0 & c(1,:)>-185.0 & c(1,:) < -179;c(:,inds) = [];
...
inds = c(1,:)<-190.0 & c(2,:)<-74.0 & c(1,:)>-200.0 & c(2,:)> -76.0;c(:,inds) = [];
inds = c(2,:)>-71.153 & c(2,:)<-71.148 & c(1,:)>-106.135 & c(1,:) < -106.11;c(:,inds) = [];
inds = c(2,:)>-64.0 & c(2,:)<-59.0 & c(1,:)>-210.0 & c(1,:) < -180.0;c(:,inds) = [];
inds = c(2,:)>-75.0 & c(2,:)<-72.5 & c(1,:)>-110.0 & c(1,:) < -75.0;c(:,inds) = [];
inds = c(2,:)>-70.8 & c(2,:)<-70.0 & c(1,:)>-78.0 & c(1,:) < -74.0;c(:,inds) = [];
inds = c(2,:)>-63.5 & c(2,:)<-63.0 & c(1,:)>-62.0 & c(1,:) < -60.0;c(:,inds) = [];
npts = length(c(1,:));
    elseif (isobath == 2500)
